...
data "aws_iam_policy_document" "lambda_execution_policy_document" {
    ...
    statement {
      ...
      effect = "Allow"
      resources = ["arn:aws:logs:${var.region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/lambda/${var.unique_id}_${var.function_name}:*"]
    }
}
...
resource "aws_iam_policy" "lambda_ssm_policy" {
  name        = "${var.unique_id}_${var.function_name}_ssm_policy"
  policy      = data.aws_iam_policy_document.lambda_ssm_policy_document.json
}
resource "aws_iam_policy" "lambda_execution_policy" {
  name        = "${var.unique_id}_${var.function_name}_execution_policy"
  policy      = data.aws_iam_policy_document.lambda_execution_policy_document.json
}
resource "aws_iam_policy" "lambda_s3_policy" {
  name        = "${var.unique_id}_${var.function_name}_s3_policy"
  policy      = data.aws_iam_policy_document.lambda_s3_policy_document.json
}
resource "aws_iam_policy" "lambda_securityhub_policy" {
  name        = "${var.unique_id}_${var.function_name}_securityhub_policy"
...
resource "aws_iam_role" "iam_for_lambda" {
  name = "${var.unique_id}_${var.function_execution_role_name}"
